...
class Sphere {
     ...
     */
    public Sphere(final int depth, final float radius,float offX,float offY,float offZ) {
        ...
        for (int stripNum = 0; stripNum < this.mTotalNumStrips; stripNum++) {
            ...
            for (int vertexNum = 0; vertexNum < numVerticesPerStrip; vertexNum += 2) {
                ...
                vertices[vertexPos++] = (float) x + offX;
                vertices[vertexPos++] = (float) y + offY;
                vertices[vertexPos++] = (float) z + offZ;
